...
ICplotdata  <- IC_raw_data %>% 
  ...
  group_by(sample) %>% 
  mutate(P = (mean(Phosphate)*1000)/94.9714, S = mean(Sulphate), Br = mean(Bromide), Fl = mean(Fluoride), Cl = mean(Chloride) , na.rm = TRUE) %>%
...
ICplot_P <- ICplotdata %>% 
  ...
  labs( x = expression(paste("P in ",mu,"mol/L")), 
        y = "Depth in cm",
...
```
``` {r ICinc, echo = FALSE}
# import photometric raw data and time information
...
pmap(list(nest_inc$data, c( "H2S", 
                         ...
                         "Br",
                         "F", 
                         "Cl")), ~ ggplot(..1 , mapping = aes(x = time , y = value, color = treated, by = Core.photometric)) + geom_point() + geom_line() + facet_wrap(~oxic_state) + labs(title = ..2))
