...
-0.227892,
-0.167105
)) %>% mutate(C_est = NH4slope*6.6, O2_umol_day = -1000*(O2slope*24)/31.9988, C_mg_day = -30*(O2slope*24)/31.9988) %>% view
...
inc_data %>%  group_by(Core.photometric) %>% summarise(PtoFe = mean(PFe, na.rm = TRUE), FeIItoFeIII = mean(FeIIFeIII, na.rm = TRUE), FeIItoFetot = mean(FeIIFetot, na.rm = TRUE)) %>% view
...
seq_extr_PFe_molten <- Seq_extr_dr %>% 
                  select("depth","Location","Incubation", PFe_MgCl_A, PFe_HCl, PFe_CDB, PFe_HNO) %>% 
                  melt(id.vars = c("depth","Location","Incubation"), variable.name = "Fraction" , na.rm = TRUE) 
...
PFe_profile_plots <- ggplot(seq_extr_PFe_molten, mapping = aes(
                        x = value,
                        y = depth ,
                        color = Location,
                        shape = Location)
      ...
      scale_y_reverse() +
       geom_point() + 
       geom_path() +
       facet_grid(Fraction ~ Incubation, scales = "free_x", labeller = Lables) +
       labs(  x = expression(paste("concentration in ",mu,"mol/L")), 
              y = "Depth in cm",
              title = "pw profiles measured with IC") +
      theme_classic() +
...
eqdepth %>% group_by(Core) %>% summarise(Location = first(Location), Incubation = first(Incubation), Pyrite = mean(Fe_HNO), sumPyrite = sum(Fe_HNO)) %>% view
