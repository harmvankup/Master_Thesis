...
ICplotdata  <- IC_raw_data %>% 
  ...
  group_by(sample) %>% 
  mutate(P = (mean(Phosphate)*1000)/94.9714, "SO" = mean(Sulphate)*1000/96.06, "Br-" = mean(Bromide)*1000/79.9, "F-" = mean(Fluoride)*1000/19, "Cl-" = mean(Chloride)*1000/35.45 ) %>%
  distinct(sample, .keep_all = TRUE) %>% 
  mutate(treated =
      case_when(
        str_detect(sample,"A|C")  ~ "treated",
        str_detect(sample,"B|D")  ~ "nontreated")) %>% full_join(
  pw_photometric,
  by = c("sample" = "Sample"),
  copy = FALSE,
...
# reshape data.
molten_IC_core_data <- ICplotdata %>% select("location", "sample", "cm_below_swi", "P.IC" : "treated","P.photometric") %>% 
melt(id.vars = c("location", "sample", "cm_below_swi", "treated"), 
     measured_.vars = c( "P.IC", 
                         "SO",      
                         "Br-",
                         "Fl-", 
                         "Cl-",
                         "P.photometric"), na.rm = TRUE)

nest_IC_pw_depth <- molten_IC_core_data %>% group_by(variable) %>% nest()

# plot profiles by parameter
pmap(
  list(nest_IC_pw_depth$data ,c("depth profile P",
                              "depth profile Sulphate",
                              "depth profile Bromide",
                              "depth profile Fluoride",
                              "depth profile Chloride",
                              "depth profile P photometric")
          ), 
  
     ~ ggplot(..1, mapping = aes(
                        x = value,
                        y = - cm_below_swi,
                        color = treated,
                        shape = location)
              ) +
        scale_color_manual( values=c("nontreated" = "blue", "treated" = "red")) +
...
ggplot(molten_IC_core_data, mapping = aes(
                        x = value,
                        y = - cm_below_swi ,
                        color = location,
                        shape = location)
...
IC_inc_data <- IC_raw_data %>% filter(str_detect(sample,'inc')) %>% group_by(sample) %>% mutate(P = (mean(Phosphate)*1000)/94.9714, S = mean(Sulphate), Br = mean(Bromide), Fl = mean(Fluoride), Cl = mean(Chloride) , na.rm = TRUE) %>% distinct(sample, .keep_all = TRUE) 
...
molten_inc <- inc_data %>% select("Core.photometric", "samplenr", "time", "H2SuM" : "DO(mg/L)" , "P":"Cl", "oxic_state", "treated") %>% 
melt(id.vars = c("Core.photometric","samplenr","time", "oxic_state", "treated"), 
     measured_.vars = c( "H2S uM", 
                         "FeII(uM)",
                         "FeTot(uM)", 
                         "FeIII(uM)",  
                         "P real (uM)", 
...
pmap(list(nest_inc$data, c( "H2S", 
                         ...
                         "Br",
                         "F", 
                         "Cl")), ~ ggplot(..1 , mapping = aes(x = time , y = value, color = treated, by = Core.photometric)) + geom_point() + geom_line() + theme(aspect.ratio = 0.5) + facet_wrap(~oxic_state) + labs(title = ..2))
